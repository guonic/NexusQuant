# -*- coding: utf-8 -*-
"""
Bottom Divergence Scanner.

Scans market data to find all bottom divergence patterns (底背离).

Usage:
    python bottom_divergence_scan.py --start-date 2024-01-01 --end-date 2024-12-31
    python bottom_divergence_scan.py --start-date 2024-01-01 --end-date 2024-12-31 --output outputs/divergence/bottom_divergence.csv --use-cache

Arguments:
    --start-date        Start date (YYYY-MM-DD)
    --end-date          End date (YYYY-MM-DD)
    --symbols           Optional comma-separated list of stock codes (e.g., "600487.SH,601688.SH")
    --output            Output CSV file path (default: outputs/divergence/bottom_divergence.csv)
    --use-cache         Use cache when loading data
    --lookback-period   Lookback period for finding previous lows (default: 30)
    --min-divergence-bars Minimum bars between two lows (default: 5)
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Scan market for bottom divergence patterns")
    parser.add_argument(
        "--start-date",
        type=str,
        required=True,
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="outputs/divergence/bottom_divergence.csv",
